 ...
 *
 */
public abstract class StricterRoundTripIT {
	...
	public StricterRoundTripIT(final String configId, final String dirName, final List<String> extensions) {
		this(configId, dirName, extensions, LocaleId.FRENCH);
	}

	public StricterRoundTripIT(final String configId, final String dirName, final List<String> extensions, final LocaleId defaultTargetLocale) {
	...
	protected void runTest(final boolean segment, final boolean detectLocales, final File file, final String configName,
			final String customConfigPath, final IFileComparator comparator) throws FileNotFoundException, URISyntaxException {
		...
		try {
			...
			assertTrue("Compare Lines: " + f, comparator.compare(Paths.get(original), Paths.get(tkitMerged)));
