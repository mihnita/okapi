...
public class RainbowTest {
	...
	public void setUp () throws URISyntaxException {
		...
		String libDir = Util.getDirectoryName(root.in("/").toString()); // Go up one dir
		...
		if ( osName.contains("OS X") ) { // Mac case
			// OS X requires -XstartOnFirstThread for GUI applications
			javaRainbow = new String[] { "java", "-XstartOnFirstThread", "-cp", libDir + "/*", "net.sf.okapi.applications.rainbow.Main" };
		} else { // Windows, Unix or Linux
			javaRainbow = new String[] { "java", "-cp", libDir + "/*", "net.sf.okapi.applications.rainbow.Main" };
		}
	}
	...
	*/
	...
	public void testTMs () throws IOException, InterruptedException {
		...
		assertTrue(deleteOutputFile("pipelines/tm/out4.mv.db")); //data.db"));
		...
		String basePath = root.in("/" + "pipelines/tm/Test01.out1.html").toString();
		String outputPath = root.in("/" + "pipelines/tm/Test01.out3.html").toString();
		assertTrue("Pensive leveraged output is different", fc.filesExactlyTheSame(outputPath, basePath));
		outputPath = root.in("/" + "pipelines/tm/Test01.out5.html").toString();
		assertTrue("Pensive leveraged output is different", fc.filesExactlyTheSame(outputPath, basePath));
	}
	...
	public void testLeverage () throws IOException, InterruptedException {
		...
		assertTrue(deleteOutputFile("pipelines/leverage/simpleTM.mv.db"));
	...
	public void testRTFConversion () throws IOException, InterruptedException {
		...
		assertTrue("File different from gold", compareWithGoldFile("pipelines/rtfConversion1.out.rtf", true));
	}

	private boolean compareWithGoldFile (String outputBase) {
		return compareWithGoldFile(outputBase, false);
	}

	private boolean compareWithGoldFile (String outputBase, boolean ignoreLineEnd) {
    	String outputPath = root.in("/" + outputBase).toString();
    	String goldPath = root.in("/" + "gold" + "/" + outputBase).toString(); 

    	return ignoreLineEnd
			? fc.compareFilesPerLines(outputPath, goldPath, "utf-8")
			: fc.filesExactlyTheSame(outputPath, goldPath);
    }
...
//	}        
    ...
    public boolean deleteOutputDir (String dirname, boolean relative) {
    	File d;
    	if ( relative ) d = root.in("/" + dirname).asFile();
    	else d = new File(dirname);
    	if ( d.isDirectory() ) {
    		String[] children = d.list();
    		for ( int i=0; i<children.length; i++ ) {
    			boolean success = deleteOutputDir(d.getAbsolutePath() + "/" + children[i], false);
    			if ( !success ) {
    				return false;
    			}
    		}
