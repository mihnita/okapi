...
public class MainForm { //implements IParametersProvider {
	...
	public static final String APPLICATIONS_RAINBOW_DESKTOP = "/.local/share/applications/rainbow.desktop";
		...
		throws Exception
	{
		...
		if (System.getProperty("os.name").contains("Linux")) {
			new MenuItem(dropMenu, SWT.SEPARATOR);
			menuItem = new MenuItem(dropMenu, SWT.PUSH);
			rm.setCommand(menuItem, "tools.installdesktopfile"); //$NON-NLS-1$
			menuItem.addSelectionListener(new SelectionAdapter() {
				public void widgetSelected(SelectionEvent event) {
		...
		inputTables = new ArrayList<>(1);
		inputTableMods = new ArrayList<>();
		...
		edSourceLang.addModifyListener(e -> updateSourceLanguageSelection());
		...
		edSourceEnc.addModifyListener(e -> updateSourceEncodingSelection());
		...
		edTargetLang.addModifyListener(e -> updateTargetLanguageSelection());
		...
		edTargetEnc.addModifyListener(e -> updateTargetEncodingSelection());
		...
		edOutputRoot.addModifyListener(e -> updateOutputRoot());
		...
		throws Exception
	{
		...
		for ( String path : paths ) {
			...
			boolean allowExpand = (Util.isEmpty(Util.getExtension(path)) || (!NOEXPAND_EXTENSIONS.contains(Util.getExtension(path))));
	...
	private void installLinuxDesktopFile() {
		String desktopLoc = System.getProperty("user.home") + APPLICATIONS_RAINBOW_DESKTOP;
		try (PrintWriter out = new PrintWriter(desktopLoc)) {
			String file = "[Desktop Entry]\n" +
					"Type=Application\n" +
					"Name=Rainbow\n" +
					"Comment=Rainbow - Okapi Localization Toolbox\n" +
					String.format("Exec=%s/rainbow.sh\n", appRootFolder) +
					String.format("Icon=%s/resources/Rainbow.png\n", appRootFolder) +
					"Terminal=false";
			out.write(file);
		} catch (Throwable e) {
			Dialogs.showError(shell, e.getMessage(), null);
