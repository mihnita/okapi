...
public class ExtractionVerificationUtil {
	...
	public boolean compareStartSubDocument(StartSubDocument ssd1, StartSubDocument ssd2) {
		...
		//--Parent Id--
		if (!bothAreNull(ssd1.getParentId(), ssd2.getParentId()) && !ssd1.getParentId().equals(ssd2.getParentId())) {
		...
		if (!bothAreNull(ssd1.getFilterParameters(), ssd2.getFilterParameters())
				&& !ssd1.getFilterParameters().equals(ssd2.getFilterParameters())) {
	...
	public boolean compareBaseReferenceable(BaseReferenceable br1, BaseReferenceable br2) {
		...
		//--Id--
		if (!bothAreNull(br1.getParentId(), br2.getParentId()) && !br1.getParentId().equals(br2.getParentId())) {
	...
	public boolean compareTextUnits(ITextUnit tu1, ITextUnit tu2) {
		...
		if (!compareIMultilingual(tu1, tu2)) {
			return false;
		}
	...
	public boolean compareTextContainers(TextContainer tc1, TextContainer tc2, boolean allowPropValChanges) {
		...
		if( !tc1.getPropertyNames().equals(tc2.getPropertyNames()) ){
			...
			for (String name : tc1.getPropertyNames()) {
				if (!compareProperties(tc1.getProperty(name), tc2.getProperty(name)) && !allowPropValChanges) {
					return false;
				}
			}
		}
	...
	public boolean compareSegments(Segment seg1, Segment seg2) {
		...
		//--Id--
		if (!bothAreNull(seg1.getId(), seg2.getId()) && !seg1.getId().equals(seg2.getId())) {
	...
	public boolean compareINameables(INameable n1, INameable n2) {
		...
		//--Name--
		if (!bothAreNull(n1.getName(), n2.getName()) && !n1.getName().equals(n2.getName())) {
			LOGGER.warn("compareINameables warning: INameable Name difference.");
			return false;
		}

		//--Type--
		if (!bothAreNull(n1.getType(), n2.getType()) && !n1.getType().equals(n2.getType())) {
			LOGGER.warn("compareINameables warning: INameable Type difference.");
			return false;
		}

		//--MimeType--
		if (!bothAreNull(n1.getMimeType(), n2.getMimeType()) && !n1.getMimeType().equals(n2.getMimeType())) {
	...
	public boolean compareIResources(IResource r1, IResource r2) {
		...
		if (!bothAreNull(r1.getId(), r2.getId()) && !r1.getId().equals(r2.getId())) {
	...
	public boolean compareIMultilingual(IMultilingual n1, IMultilingual n2) {
		...
		// --TARGET LOCALE CHECK--
		if (!n1.getTargetLocales().equals(n2.getTargetLocales()) && !isMultilingual()) {
			LOGGER.warn("compareINameables warning: INameable target locales difference.");
			return false;
		}

		// TODO: For multilingual: might need to check a possible added target in the
		// second run and compare to source props
	...
	public boolean compareProperties(Property p1, Property p2) {
		...
		if (!bothAreNull(p1.getName(), p2.getName()) && !p1.getName().equals(p2.getName())) {
			LOGGER.warn("compareProperty warning: Property name difference.");
			return false;
		}
		if (!bothAreNull(p1.getValue(), p2.getValue()) && !p1.getValue().equals(p2.getValue())) {
			LOGGER.warn("compareProperty warning: Property value difference. [prop name:{}\tval p1:{}\tval p2:{}]",
					p1.getName(), p1.getValue(), p2.getValue());
			return false;
		}
