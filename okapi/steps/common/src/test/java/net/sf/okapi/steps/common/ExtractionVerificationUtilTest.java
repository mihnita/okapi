...
public class ExtractionVerificationUtilTest {
	...
	public void testTextUnits() {
		...
		tu1.createTarget(LocaleId.FRENCH, true, INameable.COPY_ALL);
		assertFalse(util.compareTextUnits(tu1, tu2));

		tu2.createTarget(LocaleId.FRENCH, true, INameable.COPY_ALL);
		assertTrue(util.compareTextUnits(tu1, tu2));
	...
	public void testCompareIMultilingualResource() {
		...
		ITextUnit bn1 = new TextUnit();
		ITextUnit bn2 = new TextUnit();

		//SOURCE LEVEL PROPS
		bn2.setSourceProperty(new Property("name1","value1"));

		//--test prop count difference--
		assertFalse(util.compareIMultilingual(bn1, bn2));

		bn1.setSourceProperty(new Property("name1","value1"));

		//--test prop count difference--
		assertTrue(util.compareIMultilingual(bn1, bn2));
