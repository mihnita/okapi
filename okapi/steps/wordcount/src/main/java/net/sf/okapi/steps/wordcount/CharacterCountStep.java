...
public class CharacterCountStep extends BasePipelineStep {
	...
	protected Event handleEndSubDocument(Event event) {
		...
		if (subDocumentCount.isAllZeros()) {
			return event;
		}
	...
	private void updateCounts(ITextUnit tu, Event event) {
		...
		if (!textUnitCount.isAllZeros()) {
			...
			if (params.getCountInBatch()) {
				batchCount = batchCount.add(textUnitCount);
			}
			if (params.getCountInBatchItems()) {
				batchItemCount = batchItemCount.add(textUnitCount);
			}
			if (params.getCountInDocuments()) {
				documentCount = documentCount.add(textUnitCount);
			}
			if (params.getCountInSubDocuments()) {
				subDocumentCount = subDocumentCount.add(textUnitCount);
			}
			if (params.getCountInGroups()) {
				groupCount = groupCount.add(textUnitCount);
			}
		}
	}
