...
public class WhitespaceCorrectorTest {
    ...
    @Test
    public void testEnglishToJapaneseNoTarget() {
        WhitespaceCorrector corrector = new WhitespaceCorrector(LocaleId.ENGLISH, LocaleId.JAPANESE,
                Collections.singleton(Punctuation.FULL_STOP), ALL_WHITESPACE);
        ITextUnit tu = makeTu("Sentence 1.  ", "Sentence 2. ", "Sentence 3.  Sentence 4.");
        final ITextUnit corrected = corrector.correctWhitespace(tu);
        // corrector should perform no modifications to text unit
        assertNull(corrected.getTarget(LocaleId.JAPANESE));
        assertEquals(0, tu.getSource().compareTo(corrected.getSource(), false));
    }

    @Test
    public void testJapaneseToEnglishNoTarget() {
        WhitespaceCorrector corrector = new WhitespaceCorrector(LocaleId.JAPANESE, LocaleId.ENGLISH,
                Collections.singleton(Punctuation.FULL_STOP), ALL_WHITESPACE);
        ITextUnit tu = makeTu("Sentence 1。", "Sentence 2\uFF0E", "Sentence 3。Sentence 4。");
        final ITextUnit corrected = corrector.correctWhitespace(tu);
        // corrector should perform no modifications to text unit
        assertNull(corrected.getTarget(LocaleId.ENGLISH));
        assertEquals(0, tu.getSource().compareTo(corrected.getSource(), false));
    }
