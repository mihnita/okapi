...
public class TranslationComparisonStep extends BasePipelineStep {
	...
	protected Event handleEndDocument (Event event) {
		...
		double avgScore1to2 = 0;
		double avgScore1to3 = 0;
		double avgScore2to3 = 0;
		double avgFuzzyScore1to2 = 0;
		double avgFuzzyScore1to3 = 0;
		double avgFuzzyScore2to3 = 0;
    	...
    	// Average scores
    	if ( itemCount > 0 ) {
    		avgScore1to2 = (double)deScoreTotal1to2 / itemCount;
    		avgScore1to3 = (double)deScoreTotal1to3 / itemCount;
    		avgScore2to3 = (double)deScoreTotal2to3 / itemCount;
    		avgFuzzyScore1to2 = (double)fmScoreTotal1to2 / itemCount;
    		avgFuzzyScore1to3 = (double)fmScoreTotal1to3 / itemCount;
    		avgFuzzyScore2to3 = (double)fmScoreTotal2to3 / itemCount;
    	}
		...
		writer.writeElementString("p", "");
    		if ( itemCount > 0 ) {
    			...
    			writer.writeRawXML("</td><td><b>"); //$NON-NLS-1$
    			writer.writeString(String.format("%.2f", (double)wcTotal / itemCount));
    			writer.writeRawXML("</b></td></tr>\n"); //$NON-NLS-1$
    			...
    			double edScoreWordTotalAvg = ((double)edScoreWordTotal/wcTotal);
    			writer.writeRawXML("<tr><td>"); //$NON-NLS-1$
    			...
    			double fmScoreWordTotalAvg = ((double)fmScoreWordTotal/wcTotal);
    			writer.writeRawXML("<tr><td>"); //$NON-NLS-1$
    	...
    	if (( itemCount > 0 ) && params.getUseDataLog() ) {
				try (FileOutputStream fos = new FileOutputStream(params.getDataLogPath(), true);
		...
		String range)
	{
		...
		// ED-Scores
		writer.writeRawXML(((itemCount == 0) ? "<td align='right'>NA</td>" : String.format("<td align='right'>%.0f</td>", (double)edBrackets[index]/itemCount*100))); //$NON-NLS-1$
		writer.writeRawXML(String.format("<td align='right'>%d</td>", edWCBrackets[index])); //$NON-NLS-1$
		writer.writeRawXML(((wcTotal == 0) ? "<td align='right'>NA</td>" : String.format("<td align='right'>%.0f</td>", (double)edWCBrackets[index]/wcTotal*100))); //$NON-NLS-1$
		writer.writeRawXML(String.format("<td align='right'>%d</td>", fmBrackets[index])); //$NON-NLS-1$
		// FM-Scores
		writer.writeRawXML(((itemCount == 0) ? "<td align='right'>NA</td>" : String.format("<td align='right'>%.0f</td>", (double)fmBrackets[index]/itemCount*100))); //$NON-NLS-1$
		writer.writeRawXML(String.format("<td align='right'>%d</td>", fmWCBrackets[index])); //$NON-NLS-1$
		writer.writeRawXML(((wcTotal == 0) ? "<td align='right'>NA</td>" : String.format("<td align='right'>%.0f</td>", (double)fmWCBrackets[index]/wcTotal*100))); //$NON-NLS-1$
		writer.writeRawXML("</tr>\n"); //$NON-NLS-1$
	}
