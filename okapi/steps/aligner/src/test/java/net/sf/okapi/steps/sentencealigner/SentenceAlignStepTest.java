...
public class SentenceAlignStepTest {
	...
	public void sentenceEnglishEnglishAlign() {
		RawDocument t = new RawDocument(root.in("/src.txt").asUri(), "UTF-8", LocaleId.ENGLISH);
		...
		try (RawDocument rd = new RawDocument(root.in("/src.txt").asInputStream(), "UTF-8", LocaleId.ENGLISH)) {
			pipeline.process(rd);
			pipeline.endBatch();
			t.close();
		}
	...
	public void sentenceAlignMultimatch() {
		RawDocument t = new RawDocument(root.in("/trgMultimatch.txt").asUri(), "UTF-8", LocaleId.fromString("pt"));
		t.setFilterConfigId("okf_plaintext");
		...
		try(RawDocument rd = new RawDocument(root.in("/srcMultimatch.txt").asInputStream(),
	...
	public void sentenceAlignMultimatchCollpasewhitespace() {
		...
		try(RawDocument rd = new RawDocument(root.in("/srcMultimatch.txt").asInputStream(), "UTF-8", LocaleId.ENGLISH)) {
			pipeline.process(rd);
			pipeline.endBatch();
			t.close();
		}
	...
	public void sentenceAlignOnetoOneOnly() {
		RawDocument in = new RawDocument(root.in("/one_to_one1.tmx").asUri(), "UTF-8", LocaleId.ENGLISH, LocaleId.FRENCH);
	...
	public void sentenceAlignOnetoOneOnlyTwo() {
		RawDocument in = new RawDocument(root.in("/one_to_one2.tmx").asUri(), "UTF-8", LocaleId.ENGLISH, LocaleId.FRENCH);
		in.setFilterConfigId("okf_tmx");
