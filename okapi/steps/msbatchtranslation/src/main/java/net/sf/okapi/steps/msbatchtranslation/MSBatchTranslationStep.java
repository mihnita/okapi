/*===========================================================================
  Copyright (C) 2011-2018 by the Okapi Framework contributors
-----------------------------------------------------------------------------
...
public class MSBatchTranslationStep extends BasePipelineStep {
	...
	protected Event handleStartBatch (Event event) {
		...
		net.sf.okapi.connectors.microsoft.Parameters prm = conn.getParameters();
	...
	private void getTranslations () {
		...
		ArrayList<String> segIds = new ArrayList<>();
		ArrayList<String> tuIds = new ArrayList<>();
		...
		for ( Event event : events ) {
			...
			if ( tu.getSource().hasBeenSegmented() ) {
				...
				for ( Segment srcSeg : tu.getSourceSegments() ) {
					...
					// There may be no more entries
					if ( entryIndex >= segIds.size() ) continue;
						...
						if ( fill || params.getAnnotate() ) {
							...
							if ( fill ) {
								...
								if ( trgSeg.text.isEmpty() ) {
									String codedText = res.target.getCodedText();
									if ( params.getUseTargetPrefix() ) {
										codedText = params.getTargetPrefix() + codedText;
									}
									trgSeg.text.setCodedText(codedText, res.target.getClonedCodes());
								}
							}
						}
				...
				// There may be no more entries
				if ( entryIndex >= tuIds.size() ) continue;
					...
					if ( fill || params.getAnnotate() ) {
						...
						if ( fill ) {
							...
							if ( trgFrag.isEmpty() ) {
								...
								trgFrag.setCodedText(codedText, res.target.getClonedCodes());
							}
						}
					}
					// Next entry will not be the first
...
//					logger.warn("Discrepancy between the number of source and translations for text unit id='{}'", tu.getId());
//					continue;
//				}
//				Segment trgSeg = null;
//
