...
public class TradosCleanupStep extends BasePipelineStep{
	...
	protected Event handleRawDocument (Event event) {
		...
		if ( count >= batchInputCount ) {
			...
			if ( sendTm ) return Event.createNoopEvent();
			else return event;
		}
	}
	...
	protected Event execute() {
		...
		try {
			...
			// Write job file
			try (BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(jobFile), StandardCharsets.UTF_16LE))) {
				//--insert 
				out.write("\uFEFF");
	...
	@Override
	public ParametersCleanup getParameters () {
		return params;
	}
