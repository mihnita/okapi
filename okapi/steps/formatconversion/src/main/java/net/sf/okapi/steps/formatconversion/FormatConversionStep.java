	...
	public Event handleEvent (Event event) {
		...
		case START_BATCH:
			...
			switch (params.getOutputFormat()) {
				case Parameters.FORMAT_PO:
					outputType = PO_OUTPUT;
					createPOWriter();
					break;
				case Parameters.FORMAT_TMX:
					outputType = TMX_OUTPUT;
					createTMXWriter();
					break;
				case Parameters.FORMAT_PENSIEVE:
					outputType = PENSIEVE_OUTPUT;
					createPensieveWriter();
					break;
				case Parameters.FORMAT_TABLE:
					outputType = TABLE_OUTPUT;
					createTableWriter();
					break;
				case Parameters.FORMAT_CORPUS:
					outputType = CORPUS_OUTPUT;
					createCorpusWriter();
					break;
				case Parameters.FORMAT_WORDTABLE:
					outputType = WORDTABLE_OUTPUT;
					createWordTableWriter();
					break;
				case Parameters.FORMAT_XLIFF:
					outputType = XLIFF_OUTPUT;
					createXliffWriter();
					break;
			}
...
//		outParams.outputGeneric = params.getUseGenericCodes();
	...
	private void startXliffOutput () {
		...
		if (params.getSingleOutput()) {
			...
			if (params.getAutoExtensions()) {
				outFile = new File(inputURI.getPath() + ".xlf");
			} else {
				outFile = new File(outputURI);
			}
		}
