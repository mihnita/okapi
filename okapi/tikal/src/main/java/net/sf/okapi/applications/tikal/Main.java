...
public class Main {
	...
	// protected final static int CMD_ADDTRANS = 13; // No longer supported
	...
	public static void main (String[] originalArgs) {
		...
		try {
			...
			for ( int i=0; i<args.size(); i++ ) {
				...
				else if ( arg.equals("-x1") ) { // non-new-skeleton based extraction
					prog.command = CMD_EXTRACT; // Keep the flag for backward compatibility
				}
				...
				else if ( arg.equals("-m1") ) { // non-new-skeleton based merge
					prog.command = CMD_MERGE; // Flag kept for backward compatibility
				}
	...
	private static void displaySummary (int fileCount, 
		...
		Timer t)
	{
		LOGGER.info("Files: {}, Errors: {}, Time: {}", fileCount, errorCount, t);
	}
...
//			return Util.getDirectoryName(Util.getDirectoryName(path));
	...
	private IPipelineStep addSegmentationStep () {
		...
		net.sf.okapi.steps.segmentation.Parameters segParams = segStep.getParameters();
	...
	private IPipelineStep addLeveragingStep () {
		...
		net.sf.okapi.steps.leveraging.Parameters levParams = levStep.getParameters();
		if ( usePensieve ) {
	...
	private void leverageFileWithMoses (RawDocument rd) {
		...
		MergingParameters params = mrgStep.getParameters();
	...
	private void extractFileToMoses (RawDocument rd) throws URISyntaxException {
		...
		if ( moses2Outputs ) {
			FilterWriterParameters p = extStep.getParameters();
