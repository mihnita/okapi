...
public class EventBuilder implements Iterator<Event> {
	...
	/**
	 * Add a single {@link Event} at the end of the current {@link Event} queue.
	 * Assume all child events have already been added in case of an {@link IReferenceable} {@link Event}
	 * as all children events must come before their parent reference.
	 * 
	 * @param event
	...
	/**
	 * Add an list of {@link Event} at the end of the current {@link Event} queue.
	 * Assume all child events have already been added in case of an {@link IReferenceable} {@link Event}
	 * as all children events must come before their parent reference.
	 * 
	 * @param events
	 *            The list of {@link Event} to be added
	 */
	public void addFilterEvents(List<Event> events) {
		filterEvents.addAll(events);
	}
	...
	public void reset(String rootId, IFilter filter) {
		this.rootId = rootId;
		subFilter = filter instanceof SubFilter; // All sub-filters subclass from this class
		...
		referencableFilterEvents = new LinkedList<>();
		filterEvents = new LinkedList<>();

		tempFilterEventStack = new Stack<>();
	...
	private void embeddedWritableProp(IMultilingual resource, PropertyTextUnitPlaceholder propOrText, String tag,
			LocaleId locale) {
	...
	private void embeddedReadonlyProp(IMultilingual resource, PropertyTextUnitPlaceholder propOrText, String tag,
			LocaleId locId) {
