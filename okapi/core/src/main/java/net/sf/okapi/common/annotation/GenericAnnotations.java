 ...
 */
public class GenericAnnotations extends InlineAnnotation implements IAnnotation, Iterable<GenericAnnotation> {
	 ...
	 * @return a new GenericAnnotations object.
	 */
	static public GenericAnnotations createFromString (String storage) {
		return new GenericAnnotations(storage);
	}
	...
	public List<GenericAnnotation> getAnnotations (String type) {
		...
		List<GenericAnnotation> res = new ArrayList<>();
	...
	public GenericAnnotation add (String type) {
		...
		if ( list == null ) list = new ArrayList<>(INITIAL_SIZE);
		list.add(ann);
		return ann;
	}
	...
	public void addAll (GenericAnnotations annotations) {
		...
		if ( list == null ) list = new ArrayList<>(annotations.size());
	...
	public void addAll (List<GenericAnnotation> newItems) {
		...
		// There is something to add: make sure we have a place where to copy
		if ( list == null ) list = new ArrayList<>(newItems.size());
	...
	public String toString () {
		// Format: [<sep><className><sep>]<baseClassData or empty><sep><dataForAnnotation1><sep>...
		
		StringBuilder sb = new StringBuilder(CLASSNAME_SEPARATOR + getClass().getCanonicalName() + CLASSNAME_SEPARATOR);
	...
	public void fromString (String storage) {
		clear();
		// Identifier prefix
		// Handle the case of not being present for backward compatibility
		if ( storage.startsWith(CLASSNAME_SEPARATOR) ) {
			int p = storage.indexOf(CLASSNAME_SEPARATOR, 1);
			if ( p > -1 ) {
				storage = storage.substring(p+1);
			}
		}
		// Storage parts
