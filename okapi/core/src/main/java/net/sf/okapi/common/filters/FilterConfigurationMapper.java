...
public class FilterConfigurationMapper extends ParametersEditorMapper implements IFilterConfigurationMapper {
	...
	public FilterConfiguration getDefaultConfigurationFromExtension (String ext) {
		...
		for ( FilterConfiguration config : configMap.values() ) {
			if ( config.extensions != null ) {
				if (config.extensions.contains(tmp)) {
					return config;
				}
			}
		}
	...
	public void updateCustomConfigurations () {
		...
		for (String r : res) {
			addCustomConfiguration(Util.getFilename(r, false));
		}
	}
	...
	private String addConfigurations (String filterClass, URLClassLoader classLoader) {
		...
		try {
			...
			LOGGER.warn("Cannot instantiate the filter '{}'. (InstantiationException)", filterClass);
			return null;
		}
		catch ( IllegalAccessException e ) {
			LOGGER.warn("Cannot instantiate the filter '{}'. (IllegalAccessException)", filterClass);
			return null;
		}
		catch ( ClassNotFoundException e ) {
			// Make this a debug log statement as this happens all the time and clutters our logs
			LOGGER.debug("Cannot instantiate the filter '{}'. (ClassNotFoundException)", filterClass);
			return null;
		}
		...
		for ( FilterConfiguration config : list ) {
			if ( config.filterClass == null ) {
				LOGGER.warn("Configuration without filter class name in '{}'", config.toString());
				config.filterClass = filterClass;
			}
			if ( config.name == null ) {
				LOGGER.warn("Configuration without name in '{}'", config.toString());
				config.name = config.toString();
			}
			if ( config.description == null ) {
				LOGGER.warn("Configuration without description in '{}'", config.toString());
