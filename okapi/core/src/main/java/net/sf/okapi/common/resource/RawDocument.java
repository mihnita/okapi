...
public class RawDocument implements IResource, Closeable {
	 ...
	 */
	public RawDocument(final CharSequence inputCharSequence, final LocaleId sourceLocale)
	{
		this();
	 ...
	 */
	public RawDocument(final CharSequence inputCharSequence, final LocaleId sourceLocale, final LocaleId targetLocale)
	{
		this();
	 ...
	 *            the source locale for this RawDocument.
	 */
	public RawDocument(final URI inputURI, final String defaultEncoding, final LocaleId sourceLocale)
	{
		this();
	 ...
	 *            the source locale for this RawDocument.
	 */
	public RawDocument(final InputStream inputStream, final String defaultEncoding, final LocaleId sourceLocale)
	{
		this();
		try {
	 ...
	 */
	public RawDocument(final URI inputURI, final String defaultEncoding, final LocaleId sourceLocale,
			final LocaleId targetLocale, final String filterConfigId)
	{
		this();
		create(inputURI, defaultEncoding, sourceLocale, targetLocale);
	...
	private void create(final CharSequence inputCharSequence, final LocaleId srcLoc, final LocaleId trgLoc)
	{
		...
		encoding = "UTF-16";
		this.srcLoc = srcLoc;
		trgLocs = new ArrayList<>(1);
		if ( trgLoc != null ) {
			trgLocs.add(trgLoc);
		}
	}

	private void create(final URI inputURI, final String defaultEncoding, final LocaleId srcLoc, final LocaleId trgLoc)
	{
		if (inputURI == null) {
	...
	private void create(final InputStream inputStream, final String defaultEncoding, final LocaleId srcLoc,
			final LocaleId trgLoc) throws IOException
	{
		if (inputStream == null) {
			throw new IllegalArgumentException("inputStream cannot be null");
		}

		createdStream = createResettableStream(inputStream);
		encoding = defaultEncoding;
		this.srcLoc = srcLoc;
		trgLocs = new ArrayList<>(1);
		if ( trgLoc != null ) {
	...
	public Reader getReader() {
		if (UNKOWN_ENCODING.equals(getEncoding())) {
	...
	private FileCachedInputStream createStream() throws IOException {
		...
		if (getInputCharSequence() != null) {
			try {
				...
				final byte[] bytes = inputCharSequence.toString().getBytes(getEncoding());
	...
	private FileCachedInputStream createResettableStream(final InputStream is) throws IOException {
	 ...
	 */
	public void setSourceLocale(final LocaleId locId) {
		srcLoc = locId;
	}
	 ...
	 * @param locId the locale to set.
	 */
	public void setTargetLocale(final LocaleId locId) {
		trgLocs.clear();
		trgLocs.add(locId);
	}
	 ...
	 */
	public void setTargetLocales(final List<LocaleId> locIds) {
		if ( locIds != null ) {
			trgLocs = locIds;
		}
		else {
	 ...
	 */
	public void setFilterConfigId(final String filterConfigId) {
	 ...
	 * @see #finalizeOutput()
	 */
	public File createOutputFile(final URI outputURI) {
		this.outputURI = outputURI;
		if ( getInputURI() != null ) {
			final String dir = Util.getDirectoryName(outputURI.getPath());
