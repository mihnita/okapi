...
public class TextUnitUtil {
			...
			ITextUnit parent)
	{
		...
		for (Code newCode : newCodes) {
					...
					if (oriCodes.get(j).getTagType() == newCode.getTagType()) {
						if (oriIndices[j] == -1) {
							...
							if (!oriCodes.get(j).isCloneable()) {
								...
								localLogger.debug("The extra code id='{}' cannot be cloned.{}",
			...
			if (oriCode == null) { // Not found in original (extra in target)
				if ((newCode.getData() == null) || (newCode.getData().length() == 0)) {
					...
					localLogger.warn("The extra target code id='{}' does not have corresponding data.{}",
							newCode.getId(), place == null ? "" : place);
				}
		...
		if (oriCodes.size() > done) {
			...
			for (int oriIndice : oriIndices) {
				if (oriIndice != -1) {
					Code code = oriCodes.get(oriIndice);
					if (addMissingCodes) {
	 ...
	 * unit.
	 * <b>WARNING: Not all filters use {@link GenericSkeleton}. Use with caution.</b>
	 * 
	 * @param source
...
//				lastCodeId = ptf.renumberCodes(++lastCodeId);
	...
	public static String restoreSegmentation(TextContainer tc, TextFragment segStorage) {
		...
		ArrayList<TextPart> list = new ArrayList<>();
	...
	public static String toText (String text, List<Code> codes) {
		...
		for ( int i=0; i<text.length(); i++ ) {
			switch ( text.charAt(i) ) {
			case TextFragment.MARKER_OPENING:
			    addCode(tmp, 'O', TextFragment.toIndex(text.charAt(++i)), codes);
			    break;
			case TextFragment.MARKER_CLOSING:
			    addCode(tmp, 'C', TextFragment.toIndex(text.charAt(++i)), codes);
			    break;
			case TextFragment.MARKER_ISOLATED:
			    addCode(tmp, 'I', TextFragment.toIndex(text.charAt(++i)), codes);
			    break;
			default:
			    tmp.append(text.charAt(i));
			}
	...
	private static void addCode(StringBuilder tmp, char c, int index, List<Code> codes) {
	    try {
