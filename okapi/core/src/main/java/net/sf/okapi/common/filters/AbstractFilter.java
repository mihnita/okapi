...
public abstract class AbstractFilter implements IFilter {
	...
	/**
	 * Each {@link IFilter} has a small set of options beyond normal configuration
	 * that gives the {@link IFilter} the needed information to properly parse the
	 * content.
	 * 
	 * @param sourceLocale     - source locale of the input document
	 * @param targetLocale     - target locale if the input document is
	 *                         multilingual.
	 * @param defaultEncoding  - assumed encoding of the input document. May be
	 *                         overridden if a different encoding is detected.
	 * @param generateSkeleton - store skeleton (non-translatable parts of the
	 *                         document) along with the extracted text.
	 */
	...
	public List<FilterConfiguration> getConfigurations() {
		List<FilterConfiguration> configs = new ArrayList<>();

		for (FilterConfiguration fc : configList) {
			configs.add(new FilterConfiguration(fc.configId, getMimeType(), getClass().getName(), fc.name,
					fc.description, fc.parametersLocation, fc.extensions));
		}
	...
	protected boolean addConfiguration(boolean clearAllExisting, String configId, String name, String description,
		...
		return configList.add(new FilterConfiguration(configId, getMimeType(), getClass().getName(), name, description,
				parametersLocation));
	}

	protected boolean addConfiguration(boolean clearAllExisting, String configId, String name, String description,
			String parametersLocation, String extensions) {
	...
	/**
	 * Gets the filter configuration mapper if available. This mapper can be used to
	 * instantiate sub-filters based on filter configurations.
	 *
	 ...
	 *
	 * @param documentName the input document name or path
	 */
	 ...
	 * 
	 * @param srcLoc the new source locale
	 */
	...
	/**
	 * @param trgLoc the target locale to set
	 */
	 ...
	 */
	protected boolean isUtf8Encoding() {return true;}
	 ...
	 */
	protected boolean isUtf8Bom() {return false;};
}
