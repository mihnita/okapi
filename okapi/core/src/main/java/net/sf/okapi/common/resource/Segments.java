...
public class Segments implements ISegments {
			...
			boolean allowEmptySegments)
	{
		...
		for ( final Range range : ranges ) {
			...
			final Segment seg = new Segment(((range.id == null) ? String.valueOf(id++) : range.id),
	...
	public int create(int start, int end) {
		final ArrayList<Range> range = new ArrayList<>();
	...
	public int getIndex(String segId) {
		...
		for (final TextPart part : parts) {
			if ( part.isSegment() ) {
				if ( segId.equals(((Segment)part).id) ) return n;
	...
	public void validateSegmentId(Segment seg) {
		...
		for ( final TextPart tmp : parts ) {
			...
			if ( Character.isDigit(((Segment)tmp).id.charAt(0)) ) {
				...
				try {
					final int val = Integer.parseInt(((Segment)tmp).id);
