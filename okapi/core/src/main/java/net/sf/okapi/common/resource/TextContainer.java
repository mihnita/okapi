...
public class TextContainer extends BaseNameable implements Iterable<TextPart>
{
	...
	public TextContainer setContentFromString (String data) {
		setHasBeenSegmentedFlag((data.charAt(0)=='1'));
		...
		boolean collapseMode)
	{
		...
		for ( int i=0; i<parts.size(); i++ ) {
			...
			if ( (parts.get(i).isSegment() && (( parts.size() > i+1 ) && !parts.get(i+1).isSegment())) && (text.toString().trim().length() == 0) ) {
				wasWS = collapseMode;
			}
			
			// Remove the part if it's empty and not a segment
			if ( (text.length() == 0) && !parts.get(i).isSegment() ) {
				parts.remove(i);
				i--; // Adjust
			}
		}
	...
	public TextContainer clone (boolean cloneProperties) {
		...
		if ( cloneProperties ) {
			IWithProperties.copy(this, newCont);
		}

		IWithAnnotations.copy(this, newCont); 
		...
		boolean whiteSpacesAreText)
	{
		for ( TextPart part : parts ) {
			if ( part.isSegment() ) {
				if ( lookInSegments && part.getContent().hasText(whiteSpacesAreText) ) return true;
			}
			else {
	 ...
	 * <b>WARNING: All TextPart annotations and Properties are lost after joining</b>
	 */
	public void joinAll () {
		...
		int partCount)
	{
		...
		if ( (parts.size() == 1) && !parts.get(0).isSegment() ) {
