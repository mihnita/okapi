...
public class GenericAnnotationsTest {
	...
	public void testSerialization () {
		...
		String storage = anns1.toString();
		GenericAnnotations anns2 = GenericAnnotations.createFromString(storage); 
		
		assertEquals("dataText", anns2.getData());
		List<GenericAnnotation> list = anns2.getAnnotations("type1");
		GenericAnnotation ann1 = list.get(0);
		assertEquals("value1", ann1.getString("field1"));
		list = anns2.getAnnotations("type2-no-fields");
		GenericAnnotation ann2 = list.get(0);
		assertEquals("type2-no-fields", ann2.getType());
	}
