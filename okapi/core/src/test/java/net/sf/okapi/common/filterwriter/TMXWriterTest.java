...
public class TMXWriterTest {
    ...
    public void testWriteTuNoAttributes() {
    	tmxWriter.writeTU(new TextFragment("SourceContent"), new TextFragment("TargetContent"), null, null);
    	tmxWriter.close();
    	String tmx = stripNewLinesAndReturns(strWriter.toString());
    	String expectedTMX = "<tu><tuv xml:lang=\"en\"><seg>SourceContent</seg></tuv>" +
    						 "<tuv xml:lang=\"fr\"><seg>TargetContent</seg></tuv></tu>";
    	assertEquals("TU element", expectedHeaderTmx + expectedTMX, tmx);
    }

    @Test
    public void testWriteTuCreationId() {
    	Map<String, String> attrs = Collections.singletonMap(TMXWriter.CREATIONID, "okapi-test");
    	tmxWriter.writeTU(new TextFragment("SourceContent"), new TextFragment("TargetContent"), null, attrs);
    	tmxWriter.close();
    	String tmx = stripNewLinesAndReturns(strWriter.toString());
    	String expectedTMX = "<tu><tuv xml:lang=\"en\"><seg>SourceContent</seg></tuv>" +
    						 "<tuv xml:lang=\"fr\" creationid=\"okapi-test\"><seg>TargetContent</seg></tuv></tu>";
    	assertEquals("TU element", expectedHeaderTmx + expectedTMX, tmx);
    }
    ...
    public void tmxWithHeaderPropAndNote() {        
		final XMLWriter xmlWriter = new XMLWriter(strWriter);
		final TMXWriter w = new TMXWriter(xmlWriter);
		w.setWriteAllPropertiesAsAttributes(false);
		final StartDocument sd = new StartDocument("1");
		...
		String tmx = strWriter.toString();
		tmx = stripNewLinesAndReturns(tmx);
		final String expectedTMX = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<tmx version=\"1.4\">"
				...
				+ "<prop type=\"x-foo\">foobar</prop>" + "<note>nutbar</note>" + "</header>" + "<body>";
    ...
    private String getProps(ITextUnit tu) {
    	...
    	for (String propName : tu.getPropertyNames()) {
    		properties.append("<prop type=\"").append(propName).append("\">").append(tu.getProperty(propName)).append("</prop>");
    	}
    	return properties.toString();
    }

    private String getTargetTuvs(ITextUnit tu) {
    	StringBuilder targetTuvs = new StringBuilder();
    	for (LocaleId langName : tu.getTargetLocales()) {
    		targetTuvs.append("<tuv xml:lang=\"").append(langName).append("\">").append("<seg>").append(tu.getTargetSegments(langName).getFirstContent().toText()).append("</seg>").append("</tuv>");
    	}
    	return targetTuvs.toString();
    }

    private String getTargetTuvsWithAttributes(ITextUnit tu) {
    	StringBuilder targetTuvs = new StringBuilder();
    	for (LocaleId langName : tu.getTargetLocales()) {
    		targetTuvs.append("<tuv xml:lang=\"").append(langName).append("\"").append(" creationdate=\"20120822T110210Z\" changeid=\"0\" changedate=\"20130501T065729Z\">").append("<seg>").append(tu.getTargetSegments(langName).getFirstContent().toText()).append("</seg>").append("</tuv>");
    	}
    	return targetTuvs.toString();
    }
