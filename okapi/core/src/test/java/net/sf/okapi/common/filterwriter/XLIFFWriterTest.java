...
public class XLIFFWriterTest {
	...
	@Test
	public void testNonstandardMetadata()
		throws IOException
	{
		writer.create(root+"out.xlf", null, locEN, null, null, "original.ext", null);
		ITextUnit tu = new TextUnit("tu1", "src1 with <>");
		...
		// add nonstandard metadata
		GenericAnnotation a = new GenericAnnotation(GenericAnnotationType.MISC_METADATA);
		a.setString("image", "http://foo/bar/tiger.png");
		a.setString("context", "http://foo/bar/context.html");
		GenericAnnotation.addAnnotation(tu, a);
		
		// make sure other GenericAnnotations are not written out as meta
		GenericAnnotation a2 = new GenericAnnotation(GenericAnnotationType.GENERIC);
		a2.setString("dummy", "test");
		GenericAnnotation.addAnnotation(tu, a2);
		...
		assertEquals("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
			...
			+ "<trans-unit id=\"tu1\">\n"
			+ "<source xml:lang=\"en\">src1 with &lt;></source>\n"
			+ "<context-group><context context-type=\"x-image\">http://foo/bar/tiger.png</context>"
			+ "<context context-type=\"x-context\">http://foo/bar/context.html</context></context-group>\n"
			+  "<note annotates=\"source\" from=\"reviewer\">A Note</note>\n"
			+ "</trans-unit>\n"
			+ "</body>\n</file>\n</xliff>\n", result);
	}
...
//TODO: Term at TU level not OK
	...
	public void testTextWithEncapsulatedCodes ()
		throws IOException
	{
		writer.getParameters().setPlaceholderMode(false);
		writer.create(root+"out.xlf", null, locEN, null, null, "original.ext", null);
		ITextUnit tu = new TextUnit("tu1");
		tu.getSource().getFirstSegment().getContent().append(TagType.OPENING, "z", "<z>");
		tu.getSource().getFirstSegment().getContent().append("s1");
		...
		throws IOException
	{
		...
		assertEquals("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
			...
			+ ((trgLocId == null) ? "x-its" : (trgLocId.equalToString("fr") ? "x-its-translate-yes" : "protected"))
			+ "\">text</mrk></g></source>\n"
			+ ((trgLocId == null) ? "" :
				("<target xml:lang=\"fr\">t1 <g id=\"1\"><mrk its:localeFilterList=\"fr\" mtype=\""
					+ (trgLocId.equalToString("fr") ? "x-its-translate-yes" : "protected"))
			+ "\">text</mrk></g></target>\n")
			+ "</trans-unit>\n"
			+ "<trans-unit id=\"2\">\n"
			+ "<source xml:lang=\"en\">t2 <g id=\"1\"><mrk its:localeFilterList=\"fr\" its:localeFilterType=\"exclude\" mtype=\""
			+ ((trgLocId == null) ? "x-its" : (trgLocId.equalToString("fr") ? "protected" : "x-its-translate-yes"))
			+ "\">text</mrk></g></source>\n"
	...
	public void testEmptyTargetWithCreateFalse() throws Exception {
		writer.getParameters().setUseSourceForTranslated(false);
		writer.getParameters().setCopySource(false);
		writer.getParameters().setCreateEmptyTarget(false);
		writer.create(root + "out.xlf", null, locEN, locFR, null, "original.ext", null);
		ITextUnit tu = new TextUnit("1");
		tu.setSourceContent(new TextFragment("hello"));
		writer.writeTextUnit(tu);
	...
	public void writesContextGroup() throws IOException {
		final EncoderManager encoderManager = encoderManager();
		final XLIFFContextGroup.Context context1 = new XLIFFContextGroup.Context(
			encoderManager,
			"test1",
			null,
			null
		);
		context1.value("value & value");
		final XLIFFContextGroup.Context context2 = new XLIFFContextGroup.Context(
			encoderManager,
			"test2",
			"match2",
			"crc2"
		...
		final XLIFFContextGroup.Context context3 = new XLIFFContextGroup.Context(
			encoderManager,
			"test3",
			"match3",
			"crc3"
		...
		final StartGroup sg = new StartGroup();
		sg.setId("sg1");
		final XLIFFContextGroup contextGroupForGroup = new XLIFFContextGroup(
			encoderManager,
			null,
			null,
			null,
		...
		contextGroupForGroup.addContext(context1);
		sg.setAnnotation(contextGroupForGroup);

		final XLIFFContextGroup contextGroupForTextUnit = new XLIFFContextGroup(
			encoderManager,
			"cgn2",
			"purpose2",
			"src2",
		...
		contextGroupForTextUnit.addContext(context2);

		final XLIFFContextGroup contextGroupForAltTrans = new XLIFFContextGroup(
			encoderManager,
			"cgn3",
			"purpose3",
			"src3",
		...
		contextGroupForAltTrans.addContext(context3);

		final ITextUnit tu = new TextUnit("tu1", "Source Text");
		tu.setTarget(locFR, new TextContainer("Target Text"));
		tu.setAnnotation(contextGroupForTextUnit);

		TextContainer tc = tu.getTarget(locFR);
		final AltTranslationsAnnotation altTrans = new AltTranslationsAnnotation();
		tc.setAnnotation(altTrans);

		final AltTranslation alt = altTrans.add(locEN, locFR, null, null, null,
			MatchType.ACCEPTED, 0, AltTranslation.ORIGIN_SOURCEDOC);
		alt.getEntry().setPreserveWhitespaces(true);
		alt.setEngine("engine");
		alt.setAltTransType("proposal");
		alt.setTarget(locFR, tc.getFirstContent());
		alt.getEntry().setAnnotation(contextGroupForAltTrans);
	...
	private EncoderManager encoderManager() {
		final EncoderManager encoderManager = new EncoderManager();
		encoderManager.setMapping(MimeTypeMapper.XLIFF_MIME_TYPE, XMLEncoder.class.getName());
		...
		encoderManager.updateEncoder(MimeTypeMapper.XLIFF_MIME_TYPE);
