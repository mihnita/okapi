...
public class StreamUtilTest {
	...
	@Test
	public void testCRC() {
		assertEquals(1018854380L, StreamUtil.calcCRC(location.in("/ParamTest01_lf.txt").asInputStream()));
		...
		assertEquals(681066369L, StreamUtil.calcCRC(location.in("/test_lf.html").asInputStream()));
	}
	...
	public void testCopyFileToFile() throws Exception {
		File temp = testFolder.newFile();
		StreamUtil.copy(location.in("/ParamTest01_lf.txt").asFile(), temp);
		new FileCompare().filesExactlyTheSame(temp.toURI(), location.in("/ParamTest01_lf.txt").asUri());
	}

	@Test
	public void testCopyFileToOutputStream() throws Exception {
		try (ByteArrayOutputStream os = new ByteArrayOutputStream()) {
			StreamUtil.copy(location.in("/ParamTest01_lf.txt").asFile(), os);
			String out = new String(os.toByteArray(), StandardCharsets.UTF_8);
			String gold = StreamUtil.streamUtf8AsString(location.in("/ParamTest01_lf.txt").asInputStream());
