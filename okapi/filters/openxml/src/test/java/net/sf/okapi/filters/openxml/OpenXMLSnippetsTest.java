...
public class OpenXMLSnippetsTest {
	...
	@Before
	public void setUp()  {
		contentFilter = new ContentFilter(new ConditionalParameters(), "dummy.xml");
	}
	...
	private ArrayList<Event> getEvents(String snippet, ParseType filetype) {
		...
		contentFilter.setUpConfig(filetype);
		contentFilter.open(new RawDocument(snippet, locENUS));
		while (contentFilter.hasNext()) {
			Event event = contentFilter.next();
			contentFilter.displayOneEvent(event);
			list.add(event);
		}
		contentFilter.close();
		return list;
		...
		for (Event event : list) {
			...
			case START_DOCUMENT:
				writer.processStartDocument(locENUS, "utf-8", null,
					contentFilter.getEncoderManager(),
