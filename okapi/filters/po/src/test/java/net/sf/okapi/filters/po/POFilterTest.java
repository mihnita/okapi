...
public class POFilterTest {
	...
	private LocaleId locRU = LocaleId.fromString("ru");
	...
	public void testTUCompleteEntry () {
		...
		assertEquals("Translator note", note.getNote(1).getNoteText());
	...
	@Test
	public void testThreePlurals () {
		RawDocument rd = new RawDocument(root.in("/plurals-2.po").asUri(), "UTF-8", locEN, locRU);
		List<Event> events = FilterTestDriver.getEvents(filter, rd, null);
		final DocumentPart dp = FilterTestDriver.getDocumentPart(events, 1);
		assertNotNull(dp);
		final Property prop = dp.getProperty(POFilter.PROPERTY_PLURALFORMS);
		assertNotNull(prop);
		assertEquals("nplurals=3; plural=n%10==1 && n%100!=11 ? 0 : n%10>=2 && n%10<=4 && (n%100<10 || n%100>=20) ? 1 : 2;",prop.getValue());
		final StartGroup group = FilterTestDriver.getGroup(events, 1);
		assertNotNull(group);
		final List<ITextUnit> textUnits = FilterTestDriver.filterTextUnits(events);
		assertEquals(3, textUnits.size());
		assertEquals("день", textUnits.get(0).getTarget(locRU).getCodedText());
		assertEquals("дня", textUnits.get(1).getTarget(locRU).getCodedText());
		assertEquals("дней", textUnits.get(2).getTarget(locRU).getCodedText());
	}
	...
	public void testMsgCtxtAsNotes() throws Exception {
		...
		ITextUnit tu = events.get(0).getTextUnit();
		assertNotNull(tu.getAnnotation(NoteAnnotation.class));
		assertNull(events.get(1).getTextUnit().getAnnotation(NoteAnnotation.class));
		NoteAnnotation note = tu.getAnnotation(NoteAnnotation.class);
		assertEquals("This is the first string", note.getNote(0).getNoteText());
		...
		tu = events.get(0).getTextUnit();
		assertNotNull(tu.getAnnotation(NoteAnnotation.class));
		assertNotNull(events.get(1).getTextUnit().getAnnotation(NoteAnnotation.class));
		note = tu.getAnnotation(NoteAnnotation.class);
		assertEquals("Context A: This is the first string", note.getNote(0).getNoteText());
		tu = events.get(1).getTextUnit();
		note = tu.getAnnotation(NoteAnnotation.class);
		assertEquals("Context B", note.getNote(0).getNoteText());

        assertEquals("Hello world", events.get(1).getTextUnit().getSource().toString());
	}

	@Test
	public void testMarkdownSubfilter() {
		String snippet = "msgctxt \"okpCtx:tu=1\"\n"
		...
		filter.getParameters().setSubfilter("okf_markdown");
		ITextUnit tu = FilterTestDriver.getTextUnit(getEvents(snippet, locEN, locFR), 1);
		assertNotNull(tu);
		...
		assertEquals("`", codes.get(0).getData());
