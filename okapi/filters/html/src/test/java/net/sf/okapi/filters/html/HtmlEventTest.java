...
public class HtmlEventTest {
	...
	@Test
	public void baseTag() {
		String snippet = "<base href=\"https://www.example.com/\" target=\"_top\">";
		ArrayList<Event> events = new ArrayList<>();
		addStartEvents(events);
		GenericSkeleton skel = new GenericSkeleton();
		final DocumentPart dp = new DocumentPart("dp", false);
		skel.add("<base href=\"");
		dp.setSourceProperty(new Property("href", "https://www.example.com/", false));
		skel.addValuePlaceholder(dp, "href", null);
		skel.add("\" target=\"_top\">");;
		dp.setType("base");
		dp.setSkeleton(skel);
		events.add(new Event(EventType.DOCUMENT_PART, dp));
		addEndEvents(events);
