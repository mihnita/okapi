...
public class TsFilter implements IFilter {
	...
	private boolean tsPartReady(String elemName) {
		return (ts.currentDocumentLocation == DocumentLocation.TS)
				&& (elemName.equals("context") || elemName.equals("message"));
	}
	...
	private boolean contextPartReady(String elemName) {
		return (ts.currentDocumentLocation == DocumentLocation.CONTEXT)
				&& (elemName.equals("context") || elemName.equals("message"));
	}
	...
	ITextUnit generateTu() {
		...
		for (XMLEvent event : eventList) {
			...
			if (event.getEventType() == XMLEvent.START_ELEMENT) {
				...
				case "translation":
					...
					if (ts.targetIsEmpty()) {
						...
						if (approvedProp.getValue().equals("yes")) {
							...
							logger.warn("Translation (Line {}) is empty. type attribute was updated to unfinished.",
				...
				if (needTargetSection() && insertTargetAfterElem(endElemName)) {
					addTargetSection(tu);
				}
		...
		for (XMLEvent event : eventList) {
			...
			if (event.getEventType() == XMLEvent.START_ELEMENT) {
				...
				case "numerusform":
					...
					if (ts.isNumerus())
						tf_target.setProperty(new Property("numerus", "yes"));
					tf_target.setSourceContent(source);
					tf_target.createTarget(trgLang, false, INameable.CREATE_EMPTY);
				...
				case "numerusform":
					...
					if (params.getUseCodeFinder()) {
						...
						// segmented
						params.getCodeFinder().process(tf_target.getSource().getFirstContent());
						params.getCodeFinder().process(tf_target.getTarget(trgLang).getFirstContent());
						// TODO: new codes may need to be escaped!!!
					}
	...
	private boolean insertTargetAfterElem(String endElemName) {
		return endElemName.equals(ts.elemBeforeTrg);
	}

	private boolean needTargetSection() {
		return !ts.noSource() && !ts.targetExists;
	}
	...
	private void procStartElemTarget(StartElement startElement, IMultilingual resource) {
		...
		while (attributes.hasNext()) {
			...
			if (attribute.getName().getLocalPart().equals("type")) {
				...
				if (attribute.getValue().equals("unfinished")) {
					skel.addValuePlaceholder(resource, Property.APPROVED,
							trgLang);
					resource.setTargetProperty(trgLang, new Property(
					...
					Property prop = new Property(attribute.getName()
							.getLocalPart(), attribute.getValue());
					resource.setTargetProperty(trgLang, prop);
				}
			} else {
				skel.append(String.format(" %s=\"%s\"", attribute.getName()
						.getLocalPart(), attribute.getValue()));
		...
		if (!typeFound) {
			skel.addValuePlaceholder(resource, Property.APPROVED, trgLang);
			resource.setTargetProperty(trgLang, new Property(Property.APPROVED,
					"yes", false));
		}
		skel.append(">");
	}
	...
	boolean nextIsSkippableEmpty(StartElement elem) {
		...
		return elementShouldBeEmpty(elemName) && nextIsEmpty(elem);
	}
	...
	private boolean nextIsEmpty(StartElement elem) {
		...
		if ((nextEvent != null) && nextEvent.isEndElement()) {
			...
			return elem.getName().getLocalPart().equals(nextEndElem.getName().getLocalPart());
		}
		return false;
	}
