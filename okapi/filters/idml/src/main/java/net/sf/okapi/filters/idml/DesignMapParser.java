...
class DesignMapParser {
    ...
    DesignMapParser(final ZipInput<XMLEventReader> zipInputReader) {
        this.zipInputReader = zipInputReader;
    }

    DesignMap parse(final ZipEntry entry) throws XMLStreamException, IOException {
        XMLEventReader eventReader = null;
        DesignMap.DesignMapBuilder designMapBuilder = new DesignMap.DesignMapBuilder();

        try {
            eventReader = zipInputReader.of(entry);
            ...
            while (eventReader.hasNext()) {
                ...
                if (MAPPING.equals(element.getName())) {
                    designMapBuilder.setMappingPartName(element.getAttributeByName(SRC).getValue());
                    continue;
                }
