...
class TextUnitElementsMapper {
    ...
    List<Event> map(List<StoryChildElement> textUnitElements) throws XMLStreamException {
        this.baseStyleRanges = baseStyleRanges(textUnitElements);
    ...
    private StyleRanges baseStyleRanges(final List<StoryChildElement> storyChildElements) {
        return storyChildElements.stream()
            .filter(e -> e instanceof StoryChildElement.StyledTextElement.Content)
            .map(e -> ((StoryChildElement.StyledTextElement.Content) e).getStyleRanges())
            .min(Comparator.comparingInt(sd -> sd.amount()))
            .orElseGet(() -> StyleRanges.defaultStyleRanges(this.eventFactory));
    }
    ...
    private boolean assembleTextFragment(List<StoryChildElement> textUnitElements, TextFragment textFragment, int inlineReferenceOpeningDepth) throws XMLStreamException {
        ...
        for (StoryChildElement storyChildElement : textUnitElements) {
            ...
            if (storyChildElement instanceof StoryChildElement.StyledTextElement.Break) {
                if (this.parameters.getExtractBreaksInline() || 0 < inlineReferenceOpeningDepth) {
    ...
    private boolean addHyperlinkTextSource(TextFragment textFragment, StoryChildElement.StyledTextReferenceElement styledTextReferenceElement, int inlineReferenceOpeningDepth) throws XMLStreamException {
        ...
        addClosingCodes(textFragment, styleRanges, inlineReferenceOpeningDepth);

        ContentCode contentCode = new ContentCode(nextCodeId++, styledTextReferenceElement.getStyleRanges(), styledTextReferenceElement);
    ...
    protected boolean addContent(TextFragment textFragment, StoryChildElement.StyledTextElement.Content content, int inlineReferenceOpeningDepth) {
        ...
        if (!baseStyleRanges.equals(styleRanges)
                && (contentCodes.size() == inlineReferenceOpeningDepth || !contentCodes.peekFirst().getStyleRanges().equals(styleRanges))) {
            // if the specified style ranges are not equal to the base ones
            // and (the code stack is greater than the inline reference opening depth
            // or the top of the code stack is not equal to the specified style ranges)

            ContentCode contentCode = new ContentCode(nextCodeId++, styleRanges, null);
            contentCodes.push(contentCode);
            addOpeningCode(textFragment, contentCode);
        }
    ...
    private void addClosingCodes(TextFragment textFragment, StyleRanges styleRanges, int inlineReferenceOpeningDepth) {
        while (contentCodes.size() > inlineReferenceOpeningDepth
                && !contentCodes.peekFirst().getStyleRanges().isSubsetOf(styleRanges)) {
            // if the code stack size is greater than the inline reference opening depth
            // and the top of the code stack is not a subset of the specified style ranges
    ...
    protected void addTranslatableContent(TextFragment textFragment, StoryChildElement.StyledTextElement.Content content) {
        for (XMLEvent event : content.innerEvents()) {
            ...
            if (event.isProcessingInstruction()) {
                addIsolatedCode(textFragment, new SpecialCharacter.Instruction(event));
                continue;
            }
    ...
    private void isolateSpecialCharacters(TextFragment textFragment, String characters) {
        for (int i = 0; i < characters.length(); i++) {
            ...
            if (parameters.getSkipDiscretionaryHyphens()
                    && SpecialCharacter.Type.DISCRETIONARY_HYPHEN == specialCharacterType) {
                continue;
            }

            if (SpecialCharacter.Type.UNSUPPORTED != specialCharacterType) {
                SpecialCharacter specialCharacter = new SpecialCharacter.Default(eventFactory.createCharacters(String.valueOf(character)));
    ...
    private Event createTextUnitEvent(List<StoryChildElement> textUnitElements, TextFragment textFragment, String id) {
        ...
        ISkeleton skeleton = new StyledTextSkeleton(textUnitElements, this.baseStyleRanges, this.codeMap);
    ...
    private Event createDocumentPartEvent(List<StoryChildElement> textUnitElements) {
        ...
        documentPart.setSkeleton(new StyledTextSkeleton(textUnitElements, this.baseStyleRanges, Collections.<Integer, Object>emptyMap()));
