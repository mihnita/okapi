...
//				}
		...
		boolean addToSkeleton)
	{
		...
		if ( tpe != null ) {
			...
			if ( targetPointerTargetNode != null ) {
		...
		if ( addToSkeleton ) {
			if (tpe == null || targetPointerTargetNode == null) {
				skel.addReference(attributeTargetPointersWithReferenceTUs.getOrDefault(attr, tu));
	...
	private boolean processElementTag (Node node) {
		...
		if ( trav.backTracking() ) {
			...
			if ( isHTML5 && "script|style".contains(node.getLocalName()) ) {
				inNoEscapeContent = false;
			}
		}
		else { // Else: Start tag
...
//				}
			...
			if ( trav.getSubFilter(null) != null ) {
				processSubFilterContent(node, trav.getSubFilter(null));
...
//TODO: probably need to adjust the LQI annotations!				
		...
		if ( !hasTargetPointer(node) ) {
			skel.addContentPlaceholder(elementTargetPointersWithReferenceTUs.getOrDefault(node, tu));
		} else {
...
//			ann.setString(GenericAnnotationType.TERM_INFO, value)
			...
			while ( (srcNode = trav.nextNode()) != null ) {
				if ( srcNode.getNodeType() == Node.ELEMENT_NODE ) {
					...
					if ( !trav.backTracking() ) {
						...
						NamedNodeMap map = srcNode.getAttributes();
						for ( int i=0; i<map.getLength(); i++ ) {
							Attr attr = (Attr)map.item(i);
							if ( trav.getTranslate(attr) ) {
								String pointer = trav.getTargetPointer(attr);
								if ( pointer != null ) {
									setTargetPointerPair(trav.getXPath(), true, attr, pointer);
								}
							}
						}
					}
	...
	private boolean isHTML5VoidElement (String name) {
		return (",area,base,br,col,command,embed,hr,img,input,keygen,link,meta,param,source,track,wbr".contains(","+name.toLowerCase()) );
	}
}
