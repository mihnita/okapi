...
public abstract class AbstractMarkupFilter extends AbstractFilter {
	...
	/**
	 * Default constructor for {@link AbstractMarkupFilter} using specified {@link AbstractMarkupEventBuilder}
	 */
	...
	protected void preProcess(Segment segment) {
		...
		if (segment instanceof Tag) {
			Tag tag = (Tag)segment;
			RULE_TYPE elementRuleTypeCandidate = getConfig().getElementRuleTypeCandidate(tag.getName());
			if (elementRuleTypeCandidate == RULE_TYPE.INLINE_ELEMENT
					|| elementRuleTypeCandidate == RULE_TYPE.INLINE_EXCLUDED_ELEMENT
	...
	private void handleAttributeSubfiltering(StartTag startTag) {
		...
		for (Attribute attr : startTag.parseAttributes()) {
			attributeMap.clear();
			RULE_TYPE attrRuleType = getConfig().findMatchingAttributeRule(startTag.getName(),
					startTag.getAttributes().populateMap(attributeMap, true), attr.getName());

			// subfilter the attribute only if it was declared in the config
			// file as a translatable attribute
			if (attrRuleType == RULE_TYPE.ATTRIBUTE_TRANS) {
				// begin attribute
				...
				try (SubFilter pcdataSubfilter = new SubFilter(pcdataFilter,
						new XMLEncoder(getEncoding(), getNewlineType(), true, true, false, QuoteMode.UNESCAPED), 0, "null", "null")) {
					eventBuilder.addFilterEvents(pcdataSubfilter.getEvents(new RawDocument(attr.getValue(), getSrcLoc())));
	...
	protected void handleEndTag(EndTag endTag) {
		...
		RULE_TYPE ruleType = updateEndTagRuleState(endTag);
	...
	private void clarifyPreserveWhitespaceState(final EndTag endTag) {
		// handle cases such as xml:space where we popped on an element while
		// processing the attributes
		if (ruleState.isPreserveWhitespaceState()
				&& ruleState.peekPreserverWhitespaceRule().ruleName.equalsIgnoreCase(endTag.getName())) {
		...
		for (Attribute attribute : startTag.parseAttributes()) {
			...
			case ATTRIBUTE_PRESERVE_WHITESPACE:
				...
				if (preserveWS || defaultWS) {
					ruleState.pushPreserverWhitespaceRule(startTag.getName(), preserveWS);
					setPreserveWhitespace(ruleState.isPreserveWhitespaceState());
					propertyOrTextUnitPlaceholders.add(createPropertyTextUnitPlaceholder(
							PlaceholderAccessType.WRITABLE_PROPERTY, attribute.getName(),
							attribute.getValue(), startTag, attribute));
