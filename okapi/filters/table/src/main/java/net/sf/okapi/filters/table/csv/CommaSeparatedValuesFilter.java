...
/**
 * Comma-Separated Values filter. Extracts text from a comma-separated values
 * table, optionally containing a header with field names and other info.
 * 
 */
public class CommaSeparatedValuesFilter extends BaseTableFilter {
	...
	public CommaSeparatedValuesFilter() {
		...
		addConfiguration(true, // Do not inherit configurations from Base Table Filter
				FILTER_CONFIG, "Table (Comma-Separated Values)",
				"Comma-separated values, optional header with field names.", "okf_table_csv.fprm", ".csv;");
		addConfiguration(false, "okf_table_catkeys", "Haiku CatKeys", "Haiku CatKeys resource files",
				"okf_table_catkeys.fprm");
		addConfiguration(false, "okf_table_src-tab-trg", "Table (Tab-Separated Values)",
				"2-column (source + target), tab separated files.", "okf_table_src-tab-trg.fprm");
	...
	protected void init() {
		...
		case Parameters.ESCAPING_MODE_DUPLICATION:
			escapedQualifierPattern = Pattern
					.compile(String.format("((%s%s)+)[^%s]|[^%s]((%s%s)+)", params.textQualifier, params.textQualifier,
							params.textQualifier, params.textQualifier, params.textQualifier, params.textQualifier));
			break;

		case Parameters.ESCAPING_MODE_BACKSLASH:
			escapedQualifierPattern = Pattern.compile(String.format("(\\\\%s)", params.textQualifier));
			break;
		}
	...
	protected TextProcessingResult extractCells(List<ITextUnit> cells, TextContainer lineContainer, long lineNum) {
		// Extract cells from the line, if no multi-line chunks, fill up the cells list,
		// if there are, fill the chunk buffer.
		...
		if (Util.isEmpty(params.fieldDelimiter)) {
		...
		for (String chunk : chunks) {
			...
			if (!trimmedChunk.contains(params.textQualifier) && !merging) {
				buffer.add(chunk);
				continue;
			}
			...
			if (trimmedChunk.equals(params.textQualifier)) {
				...
				trimmedChunk = RegexUtil.replaceAll(trimmedChunk, escapedQualifierPattern, 1, ESCAPED_QUALIFIER);
				trimmedChunk = RegexUtil.replaceAll(trimmedChunk, escapedQualifierPattern, 3, ESCAPED_QUALIFIER);
			...
			if (level > 0) {
				...
				if (level == saveLevel && endsQualified && numTrailingQ == 0) {
					level--;
				}
	...
	protected boolean processTU(ITextUnit textUnit) {
		...
		// No // Change 2 quotes inside the field to one quote (2 adjacent quotes in CSV
		// are part of quoted text, not field qualifiers)
		if (params.removeQualifiers) {
			String st = src.getCodedText();
			// String qq = params.textQualifier + params.textQualifier;
			String qq = params.escapingMode == Parameters.ESCAPING_MODE_BACKSLASH ? "\\" + params.textQualifier
			...
			do {
				...
				src.changeToCode(start + index, start + index + qqLen, TagType.PLACEHOLDER, "CSV text qualifier"); // Quotation
																													// mark
																													// in
																													// the
																													// text
	...
	private void processBuffer(boolean forceEnding) {
		// Scans the buffer for a line, merges chunks, removes and returns the line's
		// chunks
		...
		while (true) {
			...
			while (true) {
				...
				if (Util.checkIndex(index + 1, buf)) {
					buf.remove(index + 1); // Line num
				}
			}
		...
		if (!Util.checkIndex(index, buffer)) {
			return; // = -1, no complete line of chunks
		}
		if (!Util.checkIndex(index + 1, buffer)) {
			return; // No line num item
		}
		...
		if (index == 0) {
			return; // No chunks before line break tag
		}

		List<ITextUnit> buf = new ArrayList<>();

		for (int i = 0; i < index; i++) {
			buf.add(TextUnitUtil.buildGenericTU(buffer.get(i)));
		}

		buffer.subList(0, index).clear();

		if (lineFlushed && !forceEnding)
		// We cannot add line break when forceEnding=true, as there's no event in the
		// queue to provide a skeleton
		{
			addLineBreak();
		}
		...
		if (lineFlushed && forceEnding) {
			...
			if (isSendListedMode()) {
				// DocumentParts in listed columns contain the skeleton, TUs don't write the
				// skeleton being isReferred = true
