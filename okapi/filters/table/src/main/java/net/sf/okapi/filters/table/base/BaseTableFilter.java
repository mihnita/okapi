...
public class BaseTableFilter extends BasePlainTextFilter {
	...
	protected TextProcessingResult processLine(TextContainer lineContainer) {
		...
		if (lineContainer == null || (!isMerging() && lineContainer.isEmpty())) {
		...
		List<ITextUnit> cells = new ArrayList<>();
		...
		if (Util.isEmpty(cells)) {
            return super.sendAsSource(lineContainer); // No chunks, process the whole line
        }
	...
	protected boolean processCells(List<ITextUnit> cells, long lineNum) {
		...
		if (params.sendColumnsMode == Parameters.SEND_COLUMNS_NONE) {
            return false;
        }
		if (cells == null) {
            return false;
	...
	private boolean isSource(int colNumber) {return (sourceColumns == null) ? false : sourceColumns.contains(colNumber);}
	private boolean isSourceId(int colNumber) {return (sourceIdColumns == null) ? false : sourceIdColumns.contains(colNumber);}
	private boolean isTarget(int colNumber) {return (targetColumns == null) ? false : targetColumns.contains(colNumber);}
	private boolean isComment(int colNumber) {return (commentColumns == null) ? false : commentColumns.contains(colNumber);}
	...
	private String getSuffixFromSourceRef(int colNum) {
		...
		if (sourceIdColumns == null) {
            return "";
        }
		int index = sourceIdColumns.indexOf(colNum);
		
		if (!Util.checkIndex(index, sourceIdSuffixes)) {
            return "";
        }
