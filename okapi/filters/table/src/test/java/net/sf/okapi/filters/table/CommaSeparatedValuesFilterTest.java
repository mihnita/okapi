...
public class CommaSeparatedValuesFilterTest {
	...
	public void testCatkeys () {
		...
		assertEquals("Comment 1", note.getNote(0).getNoteText());
	...
	@Test
	public void testThreeColumnsSrcTrgData_3 () throws IOException {
		
		InputStream input = root.in("/CSVTest_97.txt").asInputStream();
		assertNotNull(input);

		String snippet = streamAsString(input);
...
//		params.detectColumnsMode = Parameters.DETECT_COLUMNS_COL_NAMES;
	...
	public void testParameters() throws URISyntaxException {
		...
		assertTrue(params.unescapeSource);
    assertTrue(params.trimLeading);
    assertTrue(params.trimTrailing);
    assertTrue(params.preserveWS);
		assertFalse(params.useCodeFinder);
		assertEquals(				
		...
		assertEquals("1", params.fieldDelimiter);
		assertEquals(1, params.columnNamesLineNum);
		assertEquals(1, params.numColumns);
		assertEquals(1, params.sendHeaderMode);
		assertEquals("1", params.textQualifier);
		assertEquals(1, params.trimMode);
		assertEquals(1, params.valuesStartLineNum);
		assertTrue(params.preserveWS);
		assertTrue(params.useCodeFinder);
...
//		assertNotNull(paramsUrl);  
		...
		assertEquals(2, params.columnNamesLineNum);
		assertEquals(2, params.numColumns);
		assertEquals(2, params.sendHeaderMode);
		assertEquals("2", params.textQualifier);
		assertEquals(2, params.trimMode);
		assertEquals(2, params.valuesStartLineNum);
		assertFalse(params.preserveWS);
		assertFalse(params.useCodeFinder);
	...
	public void testSkeleton () {
		...
		try {
			st = getSkeleton(getFullFileName("/" + fileName));
		} 
		catch (UnsupportedEncodingException e) {
		}	
		LOGGER.trace("Skeleton of {}\n---\n{}\n----------", fileName, st);
		
		try {
			expected = streamAsString(root.in("/" + fileName).asInputStream());			
		} 
		catch (IOException e) {
		}
...
//		params.sendHeaderMode = 0;
	...
	public void testFileEvents96() {
		...
		params.loadFromResource(getClass(), "/okf_table@copy-of-csv_96.fprm");
		params.sendHeaderMode = Parameters.SEND_HEADER_ALL;
	...
	public void testFileEvents97() {
		...
		params.loadFromResource(getClass(), "/okf_table@copy-of-csv_97.fprm");
		params.sendHeaderMode = Parameters.SEND_HEADER_ALL;
...
//		root = Util.getDirectoryName(root) + "/data/";
		...
		throws IOException
	{
		...
		assertNotNull(tu.getAnnotation(NoteAnnotation.class));
		NoteAnnotation note = tu.getAnnotation(NoteAnnotation.class);
		assertEquals("Comment4", note.getNote(0).getNoteText());
	}
...
//		root = Util.getDirectoryName(root) + "/data/";
	...
	private String getSkeleton (String fileName) throws UnsupportedEncodingException {
		...
		return new String(writerBuffer.toByteArray(), StandardCharsets.UTF_16);
	}
