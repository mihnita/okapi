...
public class MosesTextFilterWriterTest {
	...
	public void testOutputFromXLIFF01 () {
		...
		String res = generateMosesOutput(getEventsFromFile(xlfFilter, root.in("/Test-XLIFF01.xlf").asUri()));
	...
	public void testFileOutputFromXLIFF01 () {
		File outFile = root.out("/Test-XLIFF01.out.txt").asFile();
		outFile.delete();

		IFilter xlfFilter = new net.sf.okapi.filters.xliff.XLIFFFilter();
		generateFileOutput(
			getEventsFromFile(xlfFilter, root.in("/Test-XLIFF01.xlf").asUri()),
			outFile);

		// Read the Moses file and compare with the expected result
		String res = generateMosesOutput(getEventsFromFile(filter, root.out("/Test-XLIFF01.out.txt").asUri()));
	...
	public void testOutputFromXLIFF02 () {
		// Read from XLIFF and generate the Moses file (in a string)
		IFilter xlfFilter = new net.sf.okapi.filters.xliff.XLIFFFilter();
		String res = generateMosesOutput(getEventsFromFile(xlfFilter, root.in("/Test-XLIFF02.xlf").asUri()));
	...
	private ArrayList<Event> getEventsFromFile (IFilter filter, URI uri) {
		ArrayList<Event> list = new ArrayList<>();
		filter.open(new RawDocument(uri, "UTF-8", locEN, locFR));
