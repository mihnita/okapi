...
//			}
	...
	private boolean processTextUnit (String startTag) {
		try {
			...
			ArrayList<TextFragment> trgFragments = new ArrayList<>();
			ArrayList<AltTranslation> altTranslations = new ArrayList<>();
			...
			while ( !done ) {
				...
				case XMLStreamConstants.START_ELEMENT:
					...
					if ( !inTU && name.equals("ut") ) {
						...
						if ( tmp != null ) {
							inTarget = trgLoc.equalToString(tmp);
						}
				...
				case XMLStreamConstants.END_ELEMENT:
					name = reader.getLocalName();
					switch (name) {
						case "Raw":  // End of document
							done = true;
							break;
						case "Body":  // End of document
							done = true;
							break;
						case "df":
							// We have to use placeholder for df because they don't match ut nesting order
							dfCount--;
							crumbs += "c";
							Code code = current.append(TagType.PLACEHOLDER, DFEND_TYPE, "",
									-1); //(inTarget ? ++trgId : ++srcId));
	 ...
	 * @param tf the fragment to update.
...
//	}
	...
	private boolean doesFileContainSegment (URI inputURI, String encoding)
		throws IOException
	{
		...
		try {
			int count = 0;
			reader = new BufferedReader(new InputStreamReader(input.getStream(), encoding));
