...
class TmxTu {
	...
	List <Property> propsBefore = new ArrayList<>();	//Propes of tmx tuv (only in the opening elements)
	GenericSkeleton skelAfter = new GenericSkeleton();			//Skeleton before tuv
	private final List <TmxTuv> tuvs = new ArrayList<>();		//List of all the tuvs
	...
	List<TmxTuv> getDuplicateTuvs(){
		List<TmxTuv> dups = new ArrayList<>();
		for (TmxTuv tmxTuv : tuvs){
	...
	int addPrimaryTextUnitEvent(int tuId, boolean processAllTargets, LinkedList<Event> queue){
		...
		for ( Property prop : propsBefore ) {
			if ("note".equals(prop.getName())) {
				NoteAnnotation notes = new NoteAnnotation();
				Note n = new Note(prop.getValue());
				notes.add(n);
