...
public class RainbowKitFilter implements IFilter {
		...
		try {
			...
			if ( queue.isEmpty() ) {
				...
				if ( !hasMoreDoc ) {
					...
					hasNext = false;
					queue.add(Event.createNoopEvent());
				}
			}
		...
		boolean generateSkeleton)
	{
		...
		Path inFilePath = inFile.toPath();
		boolean isPackage = inFilePath.endsWith(RAINBOWKIT_PACKAGE_EXTENSION);
		...
		if ( isPackage ) {
			Path root = inFilePath.getParent();
			String packageName = Util.getFilename(inFilePath.toString(), false);
			Path manifestFullPath = root.resolve(packageName);
			// manifestFullPath is the root (for now): unzip the package there
			try {
				ZipUtil.safeUnzip(inFilePath, manifestFullPath);
			}
			catch (IOException e) {
				throw new OkapiIOException("Error unzipping package", e);
			}
			// The add the filename
			manifestFullPath = manifestFullPath.resolve(Manifest.MANIFEST_FILENAME + Manifest.MANIFEST_EXTENSION);
			inFile = manifestFullPath.toFile();
		}
	...
	private void startDocument () {
		...
		switch (info.getExtractionType()) {
			case Manifest.EXTRACTIONTYPE_XLIFF:
				filter = new XLIFFFilter();
				extension = ".xlf";
				break;
			case Manifest.EXTRACTIONTYPE_OMEGAT:
				filter = new XLIFFFilter();
				extension = ".xlf";
				break;
			case Manifest.EXTRACTIONTYPE_PO:
				filter = new POFilter();
				extension = ".po";
				break;
			case Manifest.EXTRACTIONTYPE_RTF:
				...
				outputDocument = new RawDocument(file.toURI(), info.getTargetEncoding(),
				...
				return;
			case Manifest.EXTRACTIONTYPE_XLIFFRTF:
