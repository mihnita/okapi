...
public class PlainTextFilterTest {
	...
	public void testNameAndMimeType() {
		assertEquals("text/plain", filter.getMimeType());
		assertEquals("okf_plaintext", filter.getName());
...
//		testFile("lf.txt", false);
	...
	public void testSkeleton () {
		...
		String fileName = "crlf_start.txt";
		try {
			st = getSkeleton(getFullFileName(fileName)); // Trailing linebreak
		} 
		...
		try {
			expected = streamAsString(location.in("/" + fileName).asInputStream());
		} 
		catch (IOException e) {
		}
	...
	public void testSkeleton2 () {
		...
		try {
			st = getSkeleton(getFullFileName(fileName)); // No trailing linebreak
		} 
		catch (UnsupportedEncodingException e) {
		}	
		LOGGER.trace("Skeleton of {}\n---\n{}\n----------", fileName, st);
	...
	public void testSynchronization() {
		...
		//-------------------------
		net.sf.okapi.filters.plaintext.Parameters params = new net.sf.okapi.filters.plaintext.Parameters(filter);
		...
		assertEquals("net.sf.okapi.filters.plaintext.spliced.Parameters", c);
		...
		IFilter filter1 = filter.getActiveSiblingFilter();
		...
		assertEquals("net.sf.okapi.filters.plaintext.paragraphs.Parameters", c);
		assertFalse(c.equals("net.sf.okapi.filters.plaintext.base.Parameters"));
		assertFalse(c.equals("net.sf.okapi.filters.plaintext.spliced.Parameters"));

		filter1 = filter.getActiveSiblingFilter();
		...
		filter.setConfiguration(RegexPlainTextFilter.FILTER_CONFIG_LINES);
		c = params.getParametersClassName();
		assertEquals("net.sf.okapi.filters.plaintext.regex.Parameters", c);
		assertFalse(c.equals("net.sf.okapi.filters.plaintext.base.Parameters"));
		assertFalse(c.equals("net.sf.okapi.filters.plaintext.spliced.Parameters"));
		
		filter.setConfiguration(BasePlainTextFilter.FILTER_CONFIG);
		c = params.getParametersClassName();
		assertEquals("net.sf.okapi.filters.plaintext.base.Parameters", c);
		assertFalse(c.equals("net.sf.okapi.filters.plaintext.paragraphs.Parameters"));
...
// Helpers
	...
	private URL getPainTextConfig(String fileName) {
