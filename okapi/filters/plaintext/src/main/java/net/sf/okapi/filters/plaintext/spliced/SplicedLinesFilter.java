...
//		super.component_create();
	...
	protected void init() {
		...
		super.init();		// Have the ancestor initialize its part in params

		// Specifics		
		if (splicedLines == null) {
			splicedLines = new ArrayList<>();
		} else {
			splicedLines.clear();
		}
	}
	...
	protected TextProcessingResult processLine(TextContainer lineContainer) {
		...
		if (lineContainer == null) {
			return super.processLine(lineContainer);
		}
		if (splicedLines == null) {
			return super.processLine(lineContainer);
		}
		...
		if (TextUnitUtil.endsWith(lineContainer.getFirstContent(), params.splicer)) {
			...
			return super.processLine(lineContainer); // Plain text filter's line processing
		}								 								
	}
	...
	private boolean mergeLines() {
		...
		if (splicedLines.isEmpty()) {
			return false;
		}
		if (params == null) {
			return false;
		}
		if (Util.isEmpty(params.splicer)) {
			return false;
		}
