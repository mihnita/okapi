...
public class RegexPlainTextFilter extends AbstractFilter {
	...
	private final RegexFilter regex;			// Regex aggregate
	...
	public void setRule(String rule, int sourceGroup, int regexOptions) {		
		...
		net.sf.okapi.filters.regex.Parameters rp = _getRegexParams();
		if (rp == null) {
            return;
        }
        ...
        params.rule = rule;
        params.sourceGroup = sourceGroup;
        params.regexOptions = regexOptions;
	}
...
// IFilter	
    ...
    public boolean hasNext() {
		return regex != null && regex.hasNext();
	}
    ...
    public Event next() {
		...
		if (event == null) {
            return event; // Returns null
        }
		
		IResource res = event.getResource();
		if (res == null) {
            return event; // Do not change event
        }
    ...
    public void open(RawDocument input, boolean generateSkeleton) {
		...
		if (input == null) {
			...
			if (regex != null) {
                regex.open(input, generateSkeleton);
            }
