...
public class SRXDocumentTest {
	...
	public void testRules () {
		...
		assertNotNull(seg);
		assertTrue(seg.getLanguage().equalToString("en"));
		assertNull(seg.getRanges()); // Null set yet
		seg.computeSegments("Mr. Holmes. The detective.");
		assertNotNull(seg.getRanges());
		assertEquals(2, seg.getRanges().size());
		seg.computeSegments("MR. Holmes. The detective.");
		assertEquals(3, seg.getRanges().size());
	...
	public void testTradosCompatibility () throws IOException {
		...
		BufferedReader reader = new BufferedReader(new InputStreamReader(
			location.in("/" + testFile + ".tmx.txt").asInputStream(), StandardCharsets.UTF_8));
...
//					assertEquals(goldText, okapiText);
	...
	@Test(expected = net.sf.okapi.common.exceptions.OkapiException.class)
	...
	public void testDefaultSegmentationRules () {
		SRXDocument srxDoc = new SRXDocument();
		srxDoc.loadRules(SRXDocument.DEFAULT_SRX_RULES);
	}
}
