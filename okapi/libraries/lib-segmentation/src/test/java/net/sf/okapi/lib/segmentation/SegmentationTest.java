...
public class SegmentationTest {
	...
	public void testTrimOptionsSettingFromFile () {
		...
		FileLocation.In in = FileLocation.fromClass(SegmentationTest.class).in("/defaultSegmentation.srx");
	...
	public void testTestWithEmbededSegmentationFile () {
		SRXDocument srxDoc = new SRXDocument();
		srxDoc.loadRules(SRXDocument.class.getResourceAsStream("okapi_default_icu4j.srx"));
		ISegmenter segter = srxDoc.compileLanguageRules(LocaleId.ENGLISH, null);
		...
		assertEquals("[Sentence one.][ And sentence two.]", fmt.printSegmentedContent(tc, true));
	}
