...
public abstract class AbstractBaseDialog implements INotifiable {
	...
	public AbstractBaseDialog(boolean sizeable) {
		super();
		style = sizeable ? SWT.CLOSE | SWT.BORDER | SWT.TITLE | SWT.RESIZE | SWT.APPLICATION_MODAL
	...
	protected void create(Shell p_Parent) {
		...
		try {
			...
			Constructor<? extends Composite> cc = pageClass.getConstructor(new Class[] { Composite.class, int.class });
	...
	protected boolean run(Shell parent, Class<? extends Composite> pageClass, String caption, Object initialData,
			IHelp help) {
