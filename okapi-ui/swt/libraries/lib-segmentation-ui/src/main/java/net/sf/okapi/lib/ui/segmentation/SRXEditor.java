...
public class SRXEditor {
		...
		IHelp helpParam)
	{
		...
		edSampleLanguage.addModifyListener(e -> updateResults(false));
		...
		edSampleText.addModifyListener(e -> updateResults(false));
		...
		boolean checkIfNeedSaving)
	{
		...
		case 1: // Just an empty default group
			srxDoc.addLanguageRule(Res.getString("SRXEditor.defaultSetName"), new ArrayList<>()); //$NON-NLS-1$
