...
public class Editor implements IParametersEditor {
	/**
	 * Color items used in selection list.
	 */
	private static final String[] COLOR_ITEMS = { "blue", "dark blue", "dark red", "green", "light blue", "light green",
		...
		}
		{
			...
			tbtmWordOptions.setText("Word Options");
			{
				...
				}
				{
					...
					listWordHightlightColorsToExclude.setItems(COLOR_ITEMS);
				}
				{
					Label lblStyles = new Label(rightOptions, SWT.NONE);
					lblStyles.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true, false, 1, 1));
				...
				}
				{
					...
					listWordColorsToExclude.setItems(COLOR_ITEMS);
				}
			}
		}
		{
...
//						}
				...
				{
					Label lblSubfilter = new Label(composite, SWT.NONE);
				...
				new Label(composite, SWT.NONE);
				{
					edSubfilter = new Text(composite, SWT.BORDER);
	...
	{		
		...
		if (params.tsWordHighlightColors !=null && !params.tsWordHighlightColors.isEmpty())
		{
		...
		{
			handleOfficeColors(params.tsWordExcludedColors,listWordColorsToExclude);
		}
		...
		{
			handleOfficeColors(params.tsExcelExcludedColors,listExcelColorsToExclude);
		}
	...
	private boolean saveData () {
		...
		if (len>0)
		{
			...
			for(int i=0;i<len;i++)
			{
				...
				if (color != null) {
					params.tsWordExcludedColors.add(color.getWordColorCode());
				}
			}
		}
		else
		...
		if (len>0)
		{
			...
			for(int i=0;i<len;i++)
			{
				...
				Color color = Color.fromName(sColor);
				if (color != null) {
					params.tsExcelExcludedColors.add(color.getExcelColorCode());
				}
			}
		}
		else
	 ...
	 *
	 * @param tsExcludedColors the excluded colors from the parameter details.
	 * @param listColorsToExclude the list to update with the appropriate color name (or code if updated by hand).
	 */
	private void handleOfficeColors(TreeSet<String> tsExcludedColors, List listColorsToExclude) {
		TreeSet<String> tsColors;
		Iterator it;
		String sRGB;
		int siz;
		...
		it = tsExcludedColors.iterator();

		while (it.hasNext()) {
			sRGB = (String) it.next();
			Color color = Color.fromCode(sRGB);
			if (color != null) {
				tsColors.add(color.getName());
			} else {
				// Handle where a specific color code is added manually in the config (e.g. 3rd party Office suites)
				tsColors.add(sRGB);
			}
		}
		siz = tsColors.size();
		if (siz > 0) {
			...
			while (it.hasNext())
				sMulti[ndx++] = (String) it.next();
			listColorsToExclude.setSelection(sMulti);
		}
	}
