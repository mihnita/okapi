...
public class Editor implements IParametersEditor {
		...
		btTmp.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				if ( currentElem.conditions == null ) {
					currentElem.conditions = new ArrayList<>();
				}
		...
		btTmp.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				if ( currentAtt.conditions == null ) {
					currentAtt.conditions = new ArrayList<>();
				}
		...
		btTmp.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				if ( currentAtt.wsPreserve == null ) {
					currentAtt.wsPreserve = new ArrayList<>();
				}
		...
		btTmp.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				if ( currentAtt.wsDefault == null ) {
					currentAtt.wsDefault = new ArrayList<>();
				}
		...
		try {
			...
			for ( String name : map.keySet() ) {
				...
				for ( String itemName : items.keySet() ) {
					switch (itemName) {
						case TaggedFilterConfiguration.RULETYPES:
							@SuppressWarnings("unchecked")
							...
							break;
						case TaggedFilterConfiguration.CONDITIONS:
							elem.conditions = parseConditions(items.get(itemName));
							break;
						case TaggedFilterConfiguration.SUBFILTER:
				...
				for ( String itemName : items.keySet() ) {
					...
					switch (itemName) {
						case TaggedFilterConfiguration.RULETYPES:
							...
							break;
						case TaggedFilterConfiguration.ALL_ELEMENTS_EXCEPT:
							att.scope = Attribute.SCOPE_ALLEXCEPT;
							att.scopeElements = makeStringList(items.get(itemName).toString());
							break;
						case TaggedFilterConfiguration.ONLY_THESE_ELEMENTS:
							att.scope = Attribute.SCOPE_ONLY;
							att.scopeElements = makeStringList(items.get(itemName).toString());
							break;
						case TaggedFilterConfiguration.PRESERVE_CONDITION:
							att.wsPreserve = parseConditions(items.get(itemName));
							break;
						case TaggedFilterConfiguration.DEFAULT_CONDITION:
	...
	private java.util.List<Condition> parseConditions (Object rawObject) {
		java.util.List<Condition> list = new ArrayList<>(1);
